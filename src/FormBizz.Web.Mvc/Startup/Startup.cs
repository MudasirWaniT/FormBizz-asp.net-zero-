...
using FormBizz.Schemas;
using FormBizz.Web.Chat.SignalR;
using FormBizz.Web.Common;
using FormBizz.Web.Resources;
using Swashbuckle.AspNetCore.Swagger;
using FormBizz.Web.IdentityServer;
using FormBizz.Web.Swagger;
using Stripe;
...
using FormBizz.Web.HealthCheck;
using Owl.reCAPTCHA;
using HealthChecksUISettings = HealthChecks.UI.Configuration.Settings;
using Microsoft.AspNetCore.Server.Kestrel.Https;
using Microsoft.Extensions.DependencyInjection.Extensions;
using FormBizz.Web.Extensions;
using FormBizz.Web.MultiTenancy;
using SecurityStampValidatorCallback = FormBizz.Identity.SecurityStampValidatorCallback;

namespace FormBizz.Web.Startup
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            return services.AddAbp<FormBizzWebMvcModule>(options =>
            {
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, ILoggerFactory loggerFactory)
        {
            ...
            if (env.IsDevelopment())
            {
                ...
                app.UseFormBizzForwardedHeaders();
            }
            else
            {
            ...
            if (FormBizzConsts.PreventNotExistingTenantSubdomains)
            {
            ...
            if (WebConsts.SwaggerUiEnabled)
            {
                ...
                {
                    options.SwaggerEndpoint(_appConfiguration["App:SwaggerEndPoint"], "FormBizz API V1");
                    options.IndexStream = () => Assembly.GetExecutingAssembly()
                        .GetManifestResourceStream("FormBizz.Web.wwwroot.swagger.ui.index.html");
        ...
        private void ConfigureSwagger(IServiceCollection services)
        {
            ...
            {
                options.SwaggerDoc("v1", new OpenApiInfo() {Title = "FormBizz API", Version = "v1"});
                ...
                var applicationXml = $"FormBizz.Application.xml";
                var applicationXmlPath = Path.Combine(AppContext.BaseDirectory, applicationXml);
                options.IncludeXmlComments(applicationXmlPath);

                var webCoreXmlFile = $"FormBizz.Web.Core.xml";
